...
describe("callUpstream", () => {
  it("sends genuine headers + ticket bearer + signature when signing", async () => {
    ...
    await callUpstream(
      ...
      { model: "m", messages: [] },
      "messages",
      { deviceSigning: true, appVersion: "0.0.148", relayBase: "https://relay", fetchFn },
    );
    expect(seen.url).toBe("https://relay/v1/messages");
    expect(seen.headers.authorization).toBe("Bearer TICKET");
    expect(seen.headers["x-mirasim-client"]).toBe("0.0.148");
  ...
  it("uses plain token and no signature when signing off", async () => {
    ...
    await callUpstream(
      ...
      { model: "m" },
      "chat",
      { deviceSigning: false, appVersion: "0.0.148", relayBase: "https://relay", fetchFn },
    );
